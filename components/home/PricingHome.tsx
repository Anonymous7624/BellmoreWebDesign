      ...
      <div className="max-w-6xl mx-auto">
        <Reveal className="text-center mb-12 max-w-2xl mx-auto">
          ...
          <p className="mt-4 text-lg text-[#5F6B73] leading-relaxed">
            Websites and apps are one-time projects. Monthly plans are for social media and content. Here
            is roughly where things start.
          </p>
        </Reveal>

        {/* One-time website and app projects */}
        <Reveal className="mb-5 flex items-center gap-3">
          <h3 className="text-sm font-bold uppercase tracking-[0.16em] text-[#1E2A38]">Website &amp; app projects</h3>
          <span className="text-xs text-[#8A939B]">one-time</span>
          <span className="h-px flex-1 bg-[#E8DED0]" />
        </Reveal>
        <Stagger className="grid md:grid-cols-3 gap-6 mb-6 items-stretch" stagger={0.12}>
          {[...WEBSITE_PLANS, ...APP_PLANS].map((plan) => (
        ...
        <div className="mb-14 grid gap-4 md:grid-cols-2">
          <Reveal className="rounded-2xl border border-[#E8DED0] bg-white px-6 py-5 shadow-soft">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8A939B]">Website pricing</p>
            <p className="mt-2 text-sm text-[#5F6B73] leading-relaxed">{WEBSITE_PRICING_NOTE}</p>
          </Reveal>
          <Reveal delay={0.1} className="rounded-2xl border border-[#E8DED0] bg-white px-6 py-5 shadow-soft">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8A939B]">App pricing</p>
            <p className="mt-2 text-sm text-[#5F6B73] leading-relaxed">{APP_PRICING_NOTE}</p>
          </Reveal>
        </div>
